...
def init_db(db_path):
    ...
    copy_cypher = f"COPY A FROM '{path}/A_big-seq.csv' ; \
        COPY B FROM '{path}/B_big-seq.csv' ; \
        COPY C FROM '{path}/C_big-seq.csv' ; \
        COPY A_B FROM '{path}/edges_A_B_big-seq.csv' ; \
        COPY B_C FROM '{path}/edges_B_C_big-seq.csv' ;"
...
def main(db_path):
    ...
    db = kuzu.Database(db_path)
    conn = kuzu.Connection(db)
    # cypher = "MATCH (a:A {id: 128})-[:A_B]->(b:B)-[:B_C]->(c:C) RETURN a.id, b.id, c.id"
    cypher = "MATCH (c:C)<-[:B_C]-(:B) RETURN COUNT(DISTINCT c) AS countOfCWithNeighbors"
    # cypher = "MATCH (c:C) where c.id < 1000 return count(c)"
    # cypher = "MATCH (c:C) return count(c)"
    # run(conn, cypher)
    profile(conn, cypher)
if __name__ == "__main__":
    db_path = "/mnt/nvme/KuzuDB/example_seq_big"
    main(db_path)
